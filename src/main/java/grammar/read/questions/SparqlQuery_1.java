...
public class SparqlQuery_1 {
    ...
    public SparqlQuery_1(String entityUrl, String property, String type, String returnType) {
    ...
    public SparqlQuery_1(String sparqlQuery) {
        this.resultSparql = executeSparqlQuery(sparqlQuery);
        parseResult(resultSparql);
    }
    ...
    private String executeSparqlQuery(String query) {
        ...
        try {
            ...
            Logger.getLogger(SparqlQuery_1.class.getName()).log(Level.SEVERE, null, ex);
...
> Best regards, */
    ...
    public static String GRADABLE_ADJECTIVE_sparql(String sparql, UriLabel uriLabel) {
        ...
        //kbs[1]="Mountain";
          ...
          newSparqlQuery = 
                  ...
                  + "?uri " + "<http://dbpedia.org/ontology/" + "areaTotal" + ">" + " ?elevation .} ORDER BY DESC(?elevation) LIMIT 1";
    ...
    public static void main(String[] args) {
        ...
        String newSparqlQuery = "PREFIX dbo: <http://dbpedia.org/ontology/>\n"
                ...
                + "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n"
                + "SELECT DISTINCT ?uri \nWHERE {\n\t?uri rdf:type dbo:Mountain .\n        "
                + "?uri dbo:locatedInArea res:Germany .\n        "
                + "?uri dbo:elevation ?elevation .\n} \nORDER BY DESC(?elevation) LIMIT 1";
        */////////////
        ...
        newSparqlQuery = "PREFIX dbo: <http://dbpedia.org/ontology/> "
                         + "PREFIX skos: <http://www.w3.org/2004/02/skos/core#> "
                         + "PREFIX dbc: <http://dbpedia.org/resource/Category:> "
                         + "PREFIX dct: <http://purl.org/dc/terms/> "
                         + "SELECT ?city WHERE { "
                         + "?m skos:broader dbc:Cities_in_Australia . "
                         + "?city dct:subject ?m ; "
                         + "dbo:populationTotal ?area } ORDER BY DESC(?area) LIMIT 1";
        
        SparqlQuery_1 sparqlGeneration = new SparqlQuery_1(newSparqlQuery);
                System.out.println(newSparqlQuery);
                 System.out.println(sparqlGeneration.getObject());

        
        //dbc:Cities_in_Germany

        SparqlQuery_1 gradableSparqlQuery = new SparqlQuery_1(sparqlGeneration.getObject(), null, FIND_LABEL, "objOfProp");
